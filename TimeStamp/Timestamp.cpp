...
string Timestamp::toString() const
{
	...
	// %06lld
	char text[32] = {0};
	snprintf(text, sizeof text, "%" PRId64 ".%06" PRId64, sec, usec);
...
string Timestamp::toFormatString() const
{
	...
	char text[100] = {0};
	snprintf(text, sizeof text, "%04d%02d%02d %02d:%02d:%02d.%06" PRId64 , st.tm_year + 1900, st.tm_mon + 1, st.tm_mday, st.tm_hour, st.tm_min, st.tm_sec, usec);
	return string(text);
}
